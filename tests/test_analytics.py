...
def test_body_fat_produces_transparent_staged_target():
    body = [
        {"measured_at": "2026-07-07T08:00:00+08:00", "weight_kg": 80.0, "body_fat_pct": 30.0},
        {"measured_at": "2026-07-06T08:00:00+08:00", "weight_kg": 81.0, "body_fat_pct": 31.0},
    ]
    profile = {
        "sex": "男", "age": 30, "height_cm": 180,
    ...
    assert analysis["lean_mass_kg"] == 56.0
    assert analysis["target_weight_range_kg"] == "70.0–73.7"
    assert analysis["recommended_loss_kg"] == "约6.3公斤"
...
def test_stage_goal_tracks_progress_and_eta_to_normal_range():
    body = [
        {"measured_at": "2026-07-07T08:00:00+08:00", "weight_kg": 80.0, "body_fat_pct": 30.0},
        {"measured_at": "2026-07-06T08:00:00+08:00", "weight_kg": 81.0, "body_fat_pct": 31.0},
        {"measured_at": "2026-01-23T08:00:00+08:00", "weight_kg": 85.0, "body_fat_pct": 32.0},
    ]
    profile = {
        "sex": "男", "age": 30, "height_cm": 180,
        "target_body_fat_low": 20, "target_body_fat_high": 24,
    ...
    # Synthetic arithmetic: lean = 80 * 0.70 = 56; stage-1 = 56 / 0.76
    assert goal["current_weight_kg"] == 80.0
    assert goal["current_body_fat_pct"] == 30.0
    assert goal["start_weight_kg"] == 85.0
    assert goal["stage1_weight_kg"] == 73.7
    assert goal["final_weight_range_kg"] == "70.0–73.7"
    assert goal["remaining_kg"] == 6.3
    # Synthetic journey = 85 - 73.7 = 11.3; progress about 44%
    assert goal["progress_pct"] == 44
    assert goal["eta_weeks"] == "16–32周"
...
def test_stage_goal_marks_completed_when_in_normal_range():
    ...
    profile = {"sex": "男", "age": 30, "height_cm": 180}
    result = build_summary([], [], body, profile=profile)
    goal = result["body_composition"]["stage_goal"]
